...
const BookedSlots = ({ pushDown }) => {
    const { placesStore, bookedSlots, userDetails } = useSelector(state => state.common);
    const selectedBookedSlots = placesStore && placesStore?.filter((items) => {
        const fbkslot = bookedSlots.filter((bitems) => ((items?.id === bitems?.place_id) && (bitems?.username === userDetails?.username)))
        if (fbkslot.length > 0) {
            return items
        }
        return false;
    ...
    return (
        <div>
            <div style={{overflow:"auto", flexWrap:"wrap",  height:"100%", display:"flex", justifyContent:"center", alignItems:"center" }} >
                {/* <div>Booked slots</div> */}
                {selectedBookedSlots && selectedBookedSlots?.map((item) => {
                    return (
                        <div className='card boxsh' style={{margin:"10px"}} key={item.id}>
